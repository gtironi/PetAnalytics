...
unsigned long long totalFrequency = 0;
int cont = 0;
...
void loop()
{
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    // processamento e impressão dos dados do giroscópio
    ...
    if (cont == 100){
      ...
      Serial.println(totalFrequency/100);
      totalFrequency = 0;
      cont = 0;
    }
    unsigned long long currentMillis = micros();

    totalFrequency = totalFrequency + (1000000.0 / (currentMillis - previousMillis));
...
ICM_20948_Status_e ICM_20948::initializeDMP(void)
{
  ...
  mySmplrt.g = 10; // ODR is computed as follows: 1.1 kHz/(1+GYRO_SMPLRT_DIV[7:0]). 19 = 55Hz. InvenSense Nucleo example uses 19 (0x13).
  mySmplrt.a = 10; // ODR is computed as follows: 1.125 kHz/(1+ACCEL_SMPLRT_DIV[11:0]). 19 = 56.25Hz. InvenSense Nucleo example uses 19 (0x13).
  //mySmplrt.g = 4; // 225Hz
  //mySmplrt.a = 4; // 225Hz
  //mySmplrt.g = 8; // 112Hz
  //mySmplrt.a = 8; // 112Hz
